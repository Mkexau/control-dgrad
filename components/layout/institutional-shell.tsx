...
// Navigation générale (contrôle, direction, etc.)
const navigationGeneral: NavItem[] = [
...
const navigationRecoupementGroups: NavGroup[] = [
  {
    title: 'Accueil',
    items: [
      { href: '/dashboard', label: 'Tableau de bord', icon: '▦', exact: true },
  ...
  },
  {
    title: 'Assujettis',
    items: [
      { href: '/recoupement/assujettis', label: 'Répertoire national', icon: '◈' },
    ],
  },
  {
...
// Navigation Service d'assiette
const navigationAssiette: NavItem[] = [
  { href: '/dashboard', label: 'Tableau de bord', icon: '◧', exact: true },
  { href: '/assiette/assujettis', label: 'Répertoire', icon: '◈' },
  { href: '/assiette/assujettis?mode=create', label: 'Nouvel assujetti', icon: '＋' },
];
...
const pageTitles: Record<string, string> = {
  ...
  assiette: 'Service d’assiette',
  analyses: 'Analyses et recoupements',
  recoupement: 'Recoupement & Ordonnancement',
  admin: 'Administration',
};

function isNavActive(href: string, pathname: string, searchParams: URLSearchParams, exact?: boolean): boolean {
...
export function InstitutionalShell({ user, children }: { user: ShellUser; children: React.ReactNode }) {
  ...
  const renderNavItem = (item: NavItem) => {
    const active = isNavActive(item.href, pathname, searchParams, item.exact) ||
      (item.href === '/assiette/assujettis' && pathname.startsWith('/assujettis/'));
    return (
      <Link
        key={item.href}
        onClick={() => setIsOpen(false)}
        href={item.href}
        className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active ? 'bg-white text-[#093b78] shadow-sm' : 'text-blue-100 hover:bg-white/10 hover:text-white'}`}
      >
        <span className={`grid h-6 w-6 place-items-center rounded-md text-sm ${active ? 'bg-blue-50 text-[#0a5db5]' : 'text-blue-200'}`}>
          {item.icon}
        </span>
        {item.label}
      </Link>
  ...
  const renderFlatNav = (items: NavItem[]) => (
    <nav className="flex-1 space-y-1 px-4 py-6" aria-label="Navigation principale">
      <p className="px-3 pb-3 text-[10px] font-semibold uppercase tracking-[0.16em] text-blue-200/65">
        Espace de travail
      </p>
  ...
  const renderGroupedNav = (groups: NavGroup[]) => (
    <nav className="flex-1 overflow-y-auto px-4 py-4 space-y-4" aria-label="Navigation principale">
      {groups.map((group) => (
        <div key={group.title}>
          <p className="px-3 pb-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-blue-200/55">
            {group.title}
          </p>
  ...
  if (isAdmin) {
    navContent = renderFlatNav([
      { href: '/dashboard', label: 'Tableau de bord', icon: '▦', exact: true },
      { href: '/admin', label: 'Administration', icon: '⚙' },
    ]);
  } else if (isRecoupement) {
    navContent = renderGroupedNav(navigationRecoupementGroups);
  } else if (isServiceAssiette) {
    navContent = renderFlatNav(navigationAssiette);
  } else {
    navContent = renderFlatNav(navigationGeneral);
  }
  ...
  return (
    <div className="institutional-shell min-h-screen bg-slate-50 text-slate-900">
      ...
      <aside className={`institutional-sidebar fixed inset-y-0 left-0 z-50 flex w-72 flex-col transition-transform lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center gap-3 border-b border-white/10 px-6 py-6 shrink-0">
          <Link href="/dashboard" onClick={() => setIsOpen(false)} className="flex items-center gap-3 transition hover:opacity-90">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-white p-1 shadow-sm">
              <Image src="/images/branding/dgrad-logo.png" alt="DGRAD" width={42} height={42} className="h-full w-full object-contain" />
            </div>
            <div>
              <p className="text-sm font-bold tracking-[0.13em] text-white">DGRAD</p>
              <p className="text-[10px] font-medium uppercase tracking-[0.12em] text-blue-200">Contrôle non fiscal</p>
            </div>
          </Link>
        </div>
        ...
        <div className="m-4 shrink-0 rounded-2xl border border-white/10 bg-white/8 p-4">
          <div className="flex items-center gap-3">
            <div className="grid h-9 w-9 place-items-center rounded-full bg-amber-300 text-xs font-bold text-[#093b78]">{initials}</div>
            <div className="min-w-0">
              <p className="truncate text-xs font-semibold text-white">{user.prenom} {user.nom}</p>
        ...
        <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/92 backdrop-blur-xl">
          <div className="flex h-[4.5rem] items-center gap-4 px-4 sm:px-6 lg:px-8">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-[#0a5db5]">DGRAD · Espace sécurisé</p>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="grid h-9 w-9 place-items-center rounded-full bg-blue-50 text-xs font-bold text-[#0a5db5]">{initials}</div>
            </div>
          </div>
        </header>
